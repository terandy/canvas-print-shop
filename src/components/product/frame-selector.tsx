        ...
        <dl className="mb-8">
            ...
            <dd className="flex flex-wrap gap-3">
                {option.values.map((value) => {
                    ...
                    const optionParams = { ...state, [option.name.toLocaleLowerCase()]: value };
                    ...
                    const noneMatch = combinations.find((combination) =>
                        combination.size === optionParams.size && combination.frame === "None"
                    )
                    ...
                    return (
                        ...
                        >
                            <Image className="object-cover h-[100px]" alt={`preview-${value}`} src={getImageOptions(value.toLowerCase())} height={100} width={100} />
                            <div className="flex flex-col">
                                <span className="text-sm">{value.toLowerCase() === "black" ? "Premium" : value}</span>
                                <span className="text-xs text-gray-800">{formattedPrice}</span>
                            </div>
                        </button>
                    );
                })}
