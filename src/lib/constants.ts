export const SHOPIFY_GRAPHQL_API_ENDPOINT = "/api/2025-01/graphql.json";

export const TAGS = {
    products: "products",
    cart: "cart",
};

export const DEFAULT_OPTION = "Default Title";

export const BASE_URL = "https://canvasprintshop.ca";

export const OPENING_HOURS = "Monday-Friday 9am-5pm EST"
export const EMAIL = { label: "info@canvasprintshop.ca", href: "mailto:info@canvasprintshop.ca" }
export const PHONE = { label: "(514) 441-2230", href: "tel:5144412230" }
export const ADDRESS = { label: "1172 Av. du Lac-Saint-Charles:: Québec, QC, G3G 2S7 :: Canada", href: "https://maps.app.goo.gl/fdPr7qQHmF4rRbuD7" }

